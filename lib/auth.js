...
function verifySignature(req, res, next) {
    ...
    var key_infos = user ? user.key_info : account.key_info;

    var key = keys ? keys[keyId] : undefined;
    var key_info = key_infos ? key_infos[keyId] : undefined;

    /* delete all the keys/key_infos we didn't use, they can be pretty tubby */
    Object.keys(keys ? keys : {}).forEach(function (k) {
        if (k === keyId)
            return;
        delete (keys[k]);
        if (key_infos)
    ...
    Object.keys(key_infos ? key_infos : {}).forEach(function (k) {
        if (k === keyId)
            return;
        delete (key_infos[k]);
    });
...
function gatherContext(req, res, next) {
    ...
    if (req.key_info && req.key_info.machine_uuid) {
        conditions.machine_uuid = req.key_info.machine_uuid;
        conditions.machine_alias = req.key_info.machine_alias;
        var ts = req.key_info.machine_tags;
        Object.keys(ts).forEach(function (t) {
            conditions['machine_tag_' + t] = ts[t];
        });
    }
